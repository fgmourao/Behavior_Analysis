function [analyse,header,data] = Video_Analyse_odor(input_settings,header,data)
...
for ii = 1:length(data)
    ...
    analyse.Arena_Full.Displacement_far{ii}        = analyse.Arena_Full.Displacement{ii}(logical(analyse.quadrants.TS{ii}(:,1)));
    analyse.Arena_Full.Accumulate_distance_far{ii} = cumsum(analyse.Arena_Full.Displacement_far{ii});
    analyse.Arena_Full.Total_distance_far{ii}      = max(analyse.Arena_Full.Accumulate_distance_far{ii}); % Total distance in cm
    
    analyse.Arena_Full.Displacement_near{ii}        =  analyse.Arena_Full.Displacement{ii}(logical(analyse.quadrants.TS{ii}(:,2)));
    analyse.Arena_Full.Accumulate_distance_near{ii} = cumsum(analyse.Arena_Full.Displacement_near{ii});
    analyse.Arena_Full.Total_distance_near{ii}      = max(analyse.Arena_Full.Accumulate_distance_near{ii}); % Total distance in cm
...
if str2num(input_settings{2, 1}) == 1
    ...
    if str2num(input_settings2{1, 1}) == 2
    ...
    if str2num(input_settings2{1, 1}) == 3
    ...
    if str2num(input_settings2{1, 1}) == 4
...
elseif str2num(input_settings{2, 1}) == 2
    ...
    variables2save = {'Total distance (cm)','Mean_Velocity (cm/s)','Movements','Time Movement (s)','Time Resting(s)','Total Time at the upper open arm (s)','Total Time at botton open arm (s)','Total Time at left closed arm (s)', 'Total Time at right closed arm (s)','Total Time at center (s)','Crossings to the upper open arm','Crossings to the botton open arm','Crossings to the left closed arm','Crossings to the right closed arm','Crossings to the center'};
    spreadsheet = nan(length(header.Filename_csv),length(variables2save));
    ...
    for kk = 1:length(header.Filename_csv)
        ...
        spreadsheet(kk,11:15) = analyse.quadrants.Total_Entries_Q{1, kk};
    end
    
    
elseif str2num(input_settings{2, 1}) == 3
    
    variables2save = {'Total distance (cm)','Mean_Velocity (cm/s)','Movements','Time Movement (s)','Time Resting(s)','Total Time far target (s)', 'Total Time near target (s)', 'Total Time at the target (s)', 'Crossings to far target','Crossings to near target','Crossings to the target','Total distance far target (cm)','Total distance near target (cm)'};
    spreadsheet = nan(length(header.Filename_csv),length(variables2save));
    ...
    for kk = 1:length(header.Filename_csv)
        ...
        spreadsheet(kk,6:8) = analyse.quadrants.Total_TS{1, kk};
        spreadsheet(kk,9:11) = analyse.quadrants.Total_Entries_Q{1, kk};
        spreadsheet(kk,12) = analyse.Arena_Full.Total_distance_far{1, kk};
        spreadsheet(kk,13) = analyse.Arena_Full.Total_distance_near{1, kk};
    end
end
